...
function initNav(sections) {
    ...
    for(let i = 1; i < sections.length - 1; i++){ // bypass section 0 and thanks slide
        ...
        if(pages.length > 1){
            ...
            for(let j = 1; j < pages.length; j++){ //j = 1: bypass cover page for each section
...
function initInPageNav(sections) {
    let pageCounter = 0;
    for(let i = 1; i < sections.length; i++){ //i = 1: bypass section 0
        let pages = sections[i].getElementsByClassName('page');
        for(let j = 0; j < pages.length; j++){
            ...
            if(j < pages.length-1 && checkPageExists(i,j+1)){
                ...
                goRight.textContent = 
                    (pages[j+1].getElementsByClassName('pageTitle')[0]) ? 
